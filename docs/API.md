...
| GET | `/my-lessons` | ✅ | student | Свои занятия (фильтры `?type=&subject=&from=&to=`), с карточкой преподавателя |
| GET | `/my-lessons/stats` | ✅ | student | Сводка: занятий/часов/долг/оплачено + разбивка по предметам |
...
> **«Мои преподаватели» (2026-07-30).** Ученик, который занимается офлайн или сам, заводит карточки тех, кого нет на платформе — зеркало ученика без аккаунта у преподавателя. Карточка хранит имя, предмет, цену и контакт; занятие ссылается на неё через `studentTeacherId`, а имя дублируется в `teacherLabel` — чтобы история пережила удаление карточки (`ON DELETE SET NULL`). Раньше преподаватель и предмет вписывались строкой заново при каждой записи.
>
> `POST /my-lessons` принимает `repeatWeekly: true` — создаёт **3 занятия** подряд по неделям (это и две следующие). Каждое — отдельная запись, лишнее удаляется по одному.
>
> Свои занятия учитываются наравне с платформенными: попадают в календарь (`?from=&to=`), в KPI «занятий на неделе» и ближайшие уроки `GET /dashboard`, в долг ученика и в `GET /students/me/progress`. **В streak и часы прогресса идут только прошедшие** (`date <= CURRENT_DATE`) — иначе план на три недели вперёд накрутил бы серию.
>
> ⚠️ **Прод-миграция `20260730000001-student-teachers`** (таблица `StudentTeachers` + `StudentLessonLogs.studentTeacherId`).
...
Сервер не может достучаться до браузера напрямую, поэтому шлёт POST на `endpoint`, который
выдал push-сервис браузера (FCM у Chrome, Mozilla у Firefox, Apple у Safari). Тело шифруется
ключами подписки — посредник содержимое не читает. VAPID-подпись доказывает push-сервису,
что отправитель тот же, кому выдавали подписку.

| Метод | Путь | Auth | Роль | Описание |
|-------|------|------|------|----------|
| GET | `/push/key` | ✅ | обе | Публичный VAPID-ключ для `pushManager.subscribe()` |
| GET | `/push/status` | ✅ | обе | Включён ли push на сервере + число подписанных устройств |
| POST | `/push/subscribe` | ✅ | обе | Сохранить подписку браузера |
| DELETE | `/push/subscribe` | ✅ | обе | Отписать это устройство (по `endpoint`) |

### POST /push/subscribe
```json
// Body — ровно то, что вернул PushSubscription.toJSON() в браузере
{ "endpoint": "https://fcm.googleapis.com/fcm/send/…",
  "keys": { "p256dh": "…", "auth": "…" } }

// Response 201 { "data": { "id": "uuid", "updated": false } }
//          200 { "data": { "id": "uuid", "updated": true  } } — endpoint уже был
```

Строка = **браузер + устройство**, а не пользователь: телефон и ноутбук дают две подписки.
`endpoint` уникален индексом — при повторной подписке того же браузера приходит тот же адрес,
и без индекса копились бы дубли, а значит и дубли уведомлений. Если endpoint уже принадлежал
другому аккаунту (на устройстве сменился пользователь), подписка **переназначается**, иначе
push уходил бы прежнему владельцу.

### Отправка
Отдельного эндпоинта нет: push уходит из `utils/notify.js` — единственной точки, через которую
в приложении рождается уведомление. Поэтому он работает для всех событий сразу.
Вызов без `await`: запись в БД уже сделана, колокольчик событие покажет, а ждать сеть
внутри обработки запроса значит замедлить ответ.

**Уборка мёртвых подписок.** Ответ `404`/`410` от push-сервиса означает, что подписки больше
не будет никогда (сняли разрешение, переустановили браузер) — строка удаляется. Прочие коды
(`429`, `5xx`, сеть) временные, подписку не трогаем. Рассылка идёт через `Promise.allSettled`:
одна протухшая подписка не должна отменить доставку на остальные устройства.

> **Env:** `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` (`mailto:` — push-сервисы
> требуют контакт отправителя). Без ключей push выключен, при старте пишется строка в лог.
> Ключи генерируются командой `npx web-push generate-vapid-keys`.
