...
> ⚠️ **Актуализация (C1/C2, 2026-06-26):** `Attendance.studentId` теперь ссылается на **`Student.id`**, а не на `User.id`. Поэтому примеры ниже вида `studentId = req.user.id` / `record.studentId !== req.user.id` устарели — student-facing фильтрует по `studentId IN getStudentIdsForUser(req.user.id)`, а проверка владения — «принадлежит ли запись одной из моих `Student`-записей». **Заглушки (C2):** у `Student.userId IS NULL` посещаемость ставится сразу `confirmed` (подтверждать некому), dual-confirmation на них не распространяется. См. [REVISION.md](../../REVISION.md) §5.1–5.2.
