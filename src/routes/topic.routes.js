...
router.post('/',            auth, isStudent, aiRateLimit, validate(createTopic), ctrl.create); // + AI-роадмап
router.delete('/:id',       auth, isStudent, ctrl.remove);
router.patch('/:id/share',  auth, isStudent, validate(shareTopic), ctrl.share);
router.post('/:id/next',    auth, isStudent, aiRateLimit, ctrl.next);
router.post('/:id/attempt', auth, isStudent, validate(submitAttempt), ctrl.attempt);
router.post('/:id/grade-open', auth, isStudent, aiRateLimit, validate(gradeOpen), ctrl.gradeOpen);
router.get('/:id/sources',            auth, isStudent, ctrl.sourcesList);
router.post('/:id/sources',           auth, isStudent, aiRateLimit, validate(sourcesReq), ctrl.sources);
router.delete('/:id/sources/:sourceId', auth, isStudent, ctrl.removeSource);

// Флеш-карточки шага + интервальное повторение (SR)
router.post('/:id/cards/generate',        auth, isStudent, aiRateLimit, validate(generateCards), cards.generate);
router.post('/:id/cards/from-text',       auth, isStudent, aiRateLimit, validate(cardsFromText), cards.fromText);
